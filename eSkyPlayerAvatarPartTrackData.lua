...
function prototype:_loadFromBuff(buff, name, nameTable)
    ...
    for e = 1, eventCount do
        ...
        if storeType == 0 then
            if eventObj:loadEvent( "mod/events/avatarPart/" .. name .. ".byte") == false then
                return false;
            end
        else
            if self.pathHeader_ == nil then 
                if eventObj:loadEvent( "mod/plans/motion/" .. self.title_ .. "/avatarPart/" .. name .. ".byte") == false then
                    return false;
                end 
            else 
                if eventObj:loadEvent(self.pathHeader_ .. "avatarPart/" .. name .. ".byte") == false then
                    return false;
                end
            end
        end
