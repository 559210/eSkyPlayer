local prototype = class("eSkyPlayerAvatarPartTrackData", require("eSkyPlayer/eSkyPlayerTrackDataBase"));
...
function prototype:ctor()
    ...
    self.trackFileType_ = definations.TRACK_FILE_TYPE.AVATAR_PART;
    self.trackType_ = definations.TRACK_TYPE.AVATAR_PART;
    self.eventsSupportted_ = {definations.EVENT_TYPE.AVATAR_PART};
...
function prototype:_loadFromBuff(buff, name, nameTable)
    ...
    for e = 1, eventCount do
        ...
        eventObj = newClass("eSkyPlayer/eSkyPlayerAvatarPartEventData");
        eventObj:initialize();
        ...
        if eventFile.storeType_ == 0 then
            if eventObj:loadEvent( "mod/events/avatarPart/" .. eventFile.name_ .. ".byte") == false then
                return false;
            end
        else
            if self.pathHeader_ == nil then 
                if eventObj:loadEvent( "mod/plans/motion/" .. self.title_ .. "/avatarPart/" .. eventFile.name_ .. ".byte") == false then
                    return false;
                end 
            else 
                if eventObj:loadEvent(self.pathHeader_ .. "avatarPart/" .. eventFile.name_ .. ".byte") == false then
                    return false;
                end
            end
        end
