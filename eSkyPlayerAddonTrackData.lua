local prototype = class("eSkyPlayerAddonTrackData", require("eSkyPlayer/eSkyPlayerTrackDataBase"));
...
function prototype:ctor()
    ...
    self.trackFileType_ = definations.TRACK_FILE_TYPE.EFFECT;
    self.trackType_ = definations.TRACK_TYPE.ADDON;
    self.eventsSupportted_ = {definations.EVENT_TYPE.ADDON};
...
function prototype:_loadFromBuff(buff, name, nameTable)
    ...
    for e = 1, eventCount do
        ...
        eventObj = newClass("eSkyPlayer/eSkyPlayerAddonEventData");
        eventObj:initialize();
        ...
        if eventFile.storeType_ == 0 then
            if eventObj:loadEvent( "mod/events/effect/" .. eventFile.name_ .. ".byte") == false then
                return false;
            end
        else
            if self.pathHeader_ == nil then 
                if eventObj:loadEvent( "mod/plans/motion/" .. self.title_ .. "/effect/" .. eventFile.name_ .. ".byte") == false then
                    return false;
                end 
            else 
                if eventObj:loadEvent(self.pathHeader_ .. "motion/" .. eventFile.name_ .. ".byte") == false then
                    return false;
                end
            end
        end
